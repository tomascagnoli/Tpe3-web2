...
class controller {
    ...
    function getArtistas($params = []) {
        if(empty($params)){
            ...
            if(!empty($artista)) {
                return $this->view->response($artista,200);
            }else{
                return $this->view->response($artista,404);
            } 
        }
    }
    ...
    function addArtista() {
        $body = $this->getData();
        if(!isset($body->artista, $body->nro_albumes, $body->pais)){
    ...
    function updateArtista($params) {
        $body = $this->getData();
        if(!isset($body->artista, $body->nro_albumes, $body->pais, $params[":ID"])){
            $this->view->response("Campos vacios", 404);
            return;
        }
        else{
    ...
    function getAlbumes($params = []) {
        if(empty($params)){
            ...
            return $this->view->response($albumes,200);
        }elseif($params[":COLUMNA"] && $params[":ORDEN"]){
            $albumes = $this->model->getAlbumesByColumnaOrden($params[":COLUMNA"], $params[":ORDEN"]);
            if(!empty($albumes)) {
                return $this->view->response($albumes,200);
            }else{
                return $this->view->response($albumes,404);
            } 
        }else{
            ...
            if(!empty($album)) {
                ...
                return $this->view->response($album,404);
            } 
        }
    }
    ...
    function addAlbum() {
        $body = $this->getData();
        if(!isset($body->id_artista, $body->album, $body->nro_canciones, $body->fecha_publicacion)){
            $this->view->response("Campos vacios", 404);
            return;
        }else{
    ...
    function updateAlbum($params) {
        $body = $this->getData();
        if(!isset($body->id_artista, $body->album, $body->nro_canciones, $body->fecha_publicacion, $params[":ID"])){
            $this->view->response("Campos vacios", 404);
            return;
        }else{
            $id_artista = $body->id_artista;
